...
<%-- 	총 게시물의 수: <c:out value="${ totalCount }"/><br>
	...
	게시글의 끝 번호: <c:out value="${ endNum }"/><br> --%>	
        ...
        <tr>
			...
			[ <a href="getListCoffee.jsp?currentPage=${ i }"><c:out value="${ i }"/></a> ]
			</c:forEach>
			</div> --%>
		<!-- pagination -->
		...
		if(currentPage > pageNumber){// 현재 페이지가 pagination의 수보다 크면
			...
			prevMark = "<li class=\"page-item\"><a class=\"page-link\" href=\"selectDrinkList.jsp?currentPage="+movePage
...
/* 			prevMark = "[ <a href=\"selectCoffeeList.jsp?currentPage="+movePage
		...
		while(movePage <= endPage){
			pageLink="<li class=\"page-item\"><a class=\"page-link\" href=\"selectDrinkList.jsp?currentPage="+ movePage +"\">"+ movePage +"</a>";
		...
		if(totalPage > endPage){
			movePage = endPage + 1;
			nextMark = "<li class=\"page-item\"><a class=\"page-link\" href=\"selectDrinkList.jsp?currentPage="+movePage
