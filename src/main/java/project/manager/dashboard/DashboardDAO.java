package project.manager.dashboard;
        ...
        try {
            ...
            String selectDrinkSales =
                    "SELECT nvl(SUM(QUANTITY),0) AS amount " +
                            "FROM cart_item " +
                            "WHERE order_flag = 'N' AND receipt_flag = 'Y' AND TRUNC(input_date) = TRUNC(sysdate - ?) " +
                            "AND item_num BETWEEN 0 AND 13";
        ...
        try {
            ...
            String selectDessertSales =
                    "SELECT nvl(SUM(QUANTITY),0) AS amount " +
                            "FROM cart_item " +
                            "WHERE order_flag = 'N' AND receipt_flag = 'Y' AND TRUNC(input_date) = TRUNC(sysdate - ?) " +
                            "AND item_num BETWEEN 13 AND 15";
